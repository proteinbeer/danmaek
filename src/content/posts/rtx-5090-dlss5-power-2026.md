...
<div class="dan-point">
  <strong>단 포인트</strong>
  <p>RTX 5090에서 DLSS 5 테스트 중 전원 커넥터가 녹은 사례가 나왔습니다. 소비전력 610W 기록과 원인 해석, 고전력 GPU 시대의 전력 관리 중요성을 정리했습니다. 겉으로는 작은 변화처럼 보여도 실제 영향은 이후 흐름을 함께 봐야 분명해지죠.</p>
</div>
...
다른 테스트에서도 DLSS 5 기능을 적용했을 때 RTX 5090의 소비전력이 눈에 띄게 높아지는 사례가 관찰됐죠. 특정 극한 환경에서는 500W를 훌쩍 넘는 수치도 보고됐습니다.
...
이번 사례 역시 DLSS 5 사용 중 높은 소비전력이 기록됐다는 사실과 커넥터 손상이 같은 시점에 나타났다는 사실이 있을 뿐이죠. 둘 사이의 직접적인 인과관계는 제조사 조사로 확인된 상태가 아니죠.
...
이 부분이 재미있죠. 소프트웨어는 마법처럼 보이지만 마지막에는 GPU의 트랜지스터가 실제로 계산하고, 전기가 흐르고, 열이 발생합니다.
...
RTX 5090 이용자라면 이번 사례 하나만 보고 DLSS를 끌 필요는 없죠. 대신 16핀 커넥터가 완전히 삽입됐는지, 케이블이 커넥터 바로 옆에서 과하게 꺾이지 않았는지 확인하는 기본 점검은 해볼 만합니다.
...
전력 사용량이 늘면 전기요금만 오르는 게 아니죠. 파워서플라이, 케이스 통풍, 실내 발열, 소음까지 같이 따라오기 때문에 실제 PC 구성 비용이 커집니다.
