...
IoT 기기는 업데이트 지원 기간이 짧고 보안 수준이 제각각이죠. 같은 내부망에 있으면 취약한 기기에서 다른 장비로 접근할 가능성이 생깁니다.
...
손님용과 IoT용 네트워크를 목적에 맞게 분리하죠. 내부망 접근 차단 옵션을 켜고 인터넷만 허용합니다. 관리용 스마트폰이 필요한 경우 설정할 때만 같은 네트워크로 이동합니다.
...
일부 스마트홈 기기는 같은 로컬 네트워크에서만 검색되므로 완전 격리하면 앱 기능이 제한될 수 있습니다. 기기 특성을 확인한 뒤 분리 수준을 정해야 하죠.
