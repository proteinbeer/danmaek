...
특정 조건에서 해가 항상 매끄럽게 유지되는지 아니면 유한한 시간 안에 특이점이 생길 수 있는지가 오랫동안 풀리지 않은 문제였습니다.
...
이번 시스템은 하나의 챗봇에게 문제를 던지고 답을 기다린 방식이 아닙니다.

수천개의 에이전트를 여러 그룹으로 나눠 서로 다른 접근법을 시도하고 중간 결과를 교환하도록 구성했습니다.
...
린 같은 증명 보조 시스템은 각 단계를 기계적으로 확인할 수 있어 계산 실수와 논리 점프를 줄이는 데 도움이 됩니다.

오픈AI는 제안한 결과를 린으로 형식화했다고 밝혔습니다.

AI가 그럴듯한 수식을 만들어낸 것인지 실제로 엄밀한 증명 구조를 완성한 것인지 판단하는 데 중요한 자료입니다.
...
저는 이번 결과가 최종 인정되든 수정이 필요하든 이미 의미가 있다고 봅니다.

AI가 기존 논문을 요약하는 단계를 넘어 새로운 증명 후보를 만들고 형식 검증까지 연결하는 연구 도구로 들어왔기 때문입니다.
...
연구자는 수백개의 접근을 직접 시도하기보다 에이전트에게 넓게 탐색하게 하고 유망한 결과를 깊게 검토하는 역할로 이동할 수 있습니다.
...
## 8. 검증 속도도 AI가 높일 수 있다

흥미로운 점은 증명을 만드는 데 AI를 쓴 것뿐 아니라 검증 과정에도 형식화 도구와 AI가 들어간다는 사실입니다.
