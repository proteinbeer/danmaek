...
방화벽 규칙 일부가 프로필별로 다르게 적용됩니다. 공유 폴더와 네트워크 검색이 공용 프로필에서 막혀 문제처럼 보일 수 있습니다.

이 원인들은 서로 비슷한 증상을 만들 수 있습니다. 그래서 인터넷 속도 측정 결과나 오류 문구 하나만 보고 바로 결론을 내리기보다 물리 연결, 주소 설정, 공유기 역할을 차례로 나눠보는 편이 좋습니다.

## 3. PC와 공유기에서 볼 부분

현재 연결된 이더넷이나 와이파이의 네트워크 프로필을 확인합니다. 집 네트워크인데 다른 PC가 안 보인다면 공용으로 잡혀 있는지 봅니다. 카페와 공항 와이파이는 개인으로 바꾸지 않는 편이 안전합니다.

여기서 중요한 것은 정상값을 하나 확보하는 것입니다. 예를 들어 다른 케이블, 다른 포트, 다른 PC처럼 정상 동작하는 조건을 하나 찾으면 문제 장비를 비교하기 쉬워집니다.

## 4. 실제로 해결하는 순서

신뢰하는 집 네트워크에서 파일 공유가 필요하면 개인 프로필을 사용합니다. 공용 장소에서는 공용 프로필을 유지합니다. 프로필을 바꾼 뒤 네트워크 검색과 방화벽 설정을 함께 확인합니다.
...
개인 네트워크라는 이름이 개인정보 보호가 더 강하다는 뜻은 아닙니다. 오히려 내부 장치 간 신뢰가 높다는 의미에 가까우므로 이름을 반대로 이해하지 않도록 주의해야 합니다.
