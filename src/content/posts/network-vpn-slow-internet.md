...
VPN을 끈 상태와 켠 상태의 속도와 핑을 각각 측정합니다. 가까운 국가와 먼 국가 서버를 비교합니다. 다른 VPN 프로토콜을 선택할 수 있다면 결과를 비교합니다.

여기서 중요한 것은 정상값을 하나 확보하는 것입니다. 예를 들어 다른 케이블, 다른 포트, 다른 PC처럼 정상 동작하는 조건을 하나 찾으면 문제 장비를 비교하기 쉬워집니다.

## 4. 실제로 해결하는 순서

일반 사용은 가까운 서버나 자동 추천 서버를 우선합니다. 필요한 앱만 VPN을 통과시키는 분할 터널링을 지원하면 활용합니다. 특정 서버만 느리다면 계정이나 PC보다 서버 혼잡을 먼저 의심합니다.
...
VPN이 느리다고 보안 기능을 무작정 끄거나 정체를 알 수 없는 무료 서버로 옮기는 것은 추천하기 어렵습니다. 속도보다 신뢰할 수 있는 서비스와 목적에 맞는 설정이 우선입니다.
