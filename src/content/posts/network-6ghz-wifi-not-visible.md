...
지원 기기에서만 6GHz를 테스트해 공유기와 클라이언트를 분리합니다. 스마트 연결 기능 때문에 SSID가 하나로 합쳐졌다면 실제 연결 대역을 확인합니다. 호환성 문제가 있으면 5GHz와 6GHz 이름을 잠시 분리해 테스트합니다.
...
와이파이 6라고 적힌 기기가 모두 6GHz를 쓰는 것은 아닙니다. 6GHz를 추가한 규격은 와이파이 6E부터이므로 제품 이름을 정확히 봐야 합니다.
