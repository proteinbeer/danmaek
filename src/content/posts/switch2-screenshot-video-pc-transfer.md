...
첫 단계는 가장 단순한 조건부터 확인하는 것입니다. 스위치 2 설정에서 데이터 관리와 화면 사진 및 동영상 관리로 이동합니다. 이 단계에서 문제가 해결되면 더 큰 설정을 건드릴 필요가 없습니다.
...
특히 기억할 점은 USB 허브를 거치면 인식이 불안정할 수 있으므로 문제가 생기면 PC와 본체를 직접 연결하는 편이 좋습니다. 또 케이블이 충전만 지원하면 장치가 나타나지 않을 수 있습니다. 두 조건을 섞어서 생각하면 정상 동작을 오류로 오해하기 쉽습니다.
