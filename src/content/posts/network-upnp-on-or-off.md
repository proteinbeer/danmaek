...
먼저 UPnP 설정 문제를 한 문장으로 정리할 수 있어야 하죠. 같은 인터넷이 느리다는 표현도 유선만 느린지, 무선도 느린지, 한 기기만 그런지에 따라 원인이 완전히 달라집니다.
...
가정용 최신 공유기에서 콘솔과 게임이 필요하다면 UPnP를 사용하되 펌웨어를 유지하죠. 서버처럼 항상 같은 포트가 필요한 장비는 수동 규칙을 따로 관리합니다. 사용하지 않는 오래된 IoT 기기와 의심스러운 프로그램을 네트워크에서 정리합니다.
...
UPnP를 켜면 무조건 위험하고 끄면 무조건 안전하다는 식으로 단순화하기 어렵습니다. 사용 환경과 공유기 상태, 내부 기기 신뢰도를 함께 봐야 하죠.
