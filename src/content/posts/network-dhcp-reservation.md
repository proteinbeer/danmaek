...
DHCP는 기기에 주소를 자동으로 빌려주기 때문에 재연결 뒤 주소가 바뀔 수 있죠. 포트포워딩이나 네트워크 공유는 대상 장비 주소가 바뀌면 설정이 깨질 수 있습니다.
...
NAS, 프린터, 홈서버처럼 주소가 고정되어야 하는 장비만 예약하죠. 예약 뒤 장비 연결을 끊었다 다시 연결해 같은 주소를 받는지 확인합니다. 포트포워딩과 공유 설정은 예약이 정상 적용된 뒤 연결합니다.
...
MAC 주소 임의화 기능을 쓰는 모바일 기기는 주소가 바뀌면 예약이 풀린 것처럼 보일 수 있죠. 이런 장비는 고정이 정말 필요한지부터 판단하는 편이 좋습니다.
