---
title: "키보드를 한 번 눌렀는데 두 글자씩 입력된다면, 교체 전에 볼 것"
description: "키보드에서 같은 글자가 두 번씩 찍히거나 한 번 누른 키가 연속 입력되는 현상은 흔히 채터링이라고 부릅니다."
date: 2026-08-22
updated: 2026-08-22
category: "가이드"
subcategory: "주변기기"
...
키를 길게 눌렀을 때 같은 문자가 빠르게 반복되는 것은 정상적인 키 반복 기능입니다.

반대로 짧게 한 번 눌렀는데 두 번이나 세 번 입력된다면 채터링을 의심할 수 있죠.

먼저 메모장처럼 단순한 프로그램에서 같은 키를 여러 번 천천히 눌러보시면 됩니다.
...
제어판의 키보드 속성에서는 반복 지연과 반복 속도를 조절할 수 있습니다.
...
허브 전력이나 연결이 불안정하면 키 입력이 끊기거나 재연결되면서 이상한 증상이 나타날 수 있습니다.

다른 PC에 연결해 같은 키를 반복해서 눌러보는 것도 빠른 확인 방법입니다.

환경을 바꿔도 특정 키에서만 문제가 반복된다면 하드웨어 쪽 가능성이 커집니다.
...
기계식 스위치 안쪽에 액체 세정제를 무리하게 넣는 방식은 피하는 편이 안전합니다.
...
## 6. 소프트웨어 채터링 보정은 임시 방법

일부 프로그램은 너무 짧은 간격으로 반복된 입력을 무시해 채터링을 줄여줍니다.

당장 키보드를 교체하기 어렵다면 쓸 수 있지만 근본적인 스위치 접점 문제를 고치는 것은 아닙니다.
...
채터링은 작은 증상이지만 비밀번호나 문서 입력에서는 상당히 스트레스를 줍니다.

설정 문제인지 스위치 문제인지 순서대로 나누면 키보드 전체를 불필요하게 바꾸는 일을 줄일 수 있습니다.
