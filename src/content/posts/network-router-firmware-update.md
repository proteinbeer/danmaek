...
오래된 펌웨어에 알려진 보안 취약점이 남을 수 있습니다. 메모리 누수나 연결 끊김 같은 안정성 문제가 업데이트로 수정되는 경우가 있습니다.

새 기기와 와이파이 호환성 문제가 개선되는 경우가 있습니다. 지원이 끝난 공유기는 더 이상 수정 패치를 받지 못할 수 있습니다.

이 원인들은 서로 비슷한 증상을 만들 수 있습니다. 그래서 인터넷 속도 측정 결과나 오류 문구 하나만 보고 바로 결론을 내리기보다 물리 연결, 주소 설정, 공유기 역할을 차례로 나눠보는 편이 좋습니다.

## 3. PC와 공유기에서 볼 부분

공유기 관리자 페이지에서 현재 펌웨어 버전과 업데이트 날짜를 확인합니다. 제조사 지원 페이지에서 해당 모델이 아직 지원 중인지 봅니다. 자동 업데이트 기능이 있다면 켜져 있는지 확인합니다.

여기서 중요한 것은 정상값을 하나 확보하는 것입니다. 예를 들어 다른 케이블, 다른 포트, 다른 PC처럼 정상 동작하는 조건을 하나 찾으면 문제 장비를 비교하기 쉬워집니다.

## 4. 실제로 해결하는 순서

업데이트 전 현재 설정을 백업할 수 있다면 저장합니다. 유선 연결 상태에서 업데이트하고 진행 중 전원을 끄지 않습니다. 지원 종료 모델이라면 보안과 성능을 고려해 교체 시점을 검토합니다.
