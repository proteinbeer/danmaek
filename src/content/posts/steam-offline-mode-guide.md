---
title: "스팀 오프라인 모드 설정하는 법, 인터넷 없이 게임 플레이"
description: "인터넷이 끊겨도 스팀에 설치된 게임을 즐길 수 있는 오프라인 모드 사용법을 정리했습니다. 전환 방법과 오프라인에서 실행할 수 없는 게임까지 함께 다룹니다."
date: 2026-08-26
updated: 2026-08-26
category: "가이드"
subcategory: "스팀"
...
imageAlt: "인터넷 없이 실행할 수 있는 스팀 게임 라이브러리"
imageWidth: 1200
imageHeight: 675
hideHeroImage: true
...
오프라인 모드는 스팀 클라이언트가 서버에 연결하지 않고도 PC에 설치된 게임을 실행할 수 있게 해주는 기능입니다.

- 온라인 상태에서 이미 설치·실행해본 게임은 오프라인에서도 대부분 그대로 실행됩니다.
- 도중에 인터넷이 끊겨도, 오프라인 모드를 켜두면 갑자기 게임이 종료되지 않고 플레이를 이어갈 수 있죠.
- 다운로드, 업데이트, 친구 목록, 클라우드 동기화 같은 온라인 기능은 오프라인 동안 동작하지 않습니다.

한 번도 실행하지 않은 게임은 오프라인 시작을 거부하는 경우가 있습니다. 미리 온라인 상태에서 그 게임을 한 번 실행해두면 오프라인에서도 안전합니다.
...
- 스팀을 실행하면 로그인 서버 연결이 실패하고 **연결 오류** 화면이 나타납니다.
...
오프라인 중에는 세이브의 클라우드 동기화가 진행되지 않습니다.

- 오프라인으로 플레이한 세이브는 PC 로컬에 저장됩니다.
- 다시 온라인으로 전환하면 클라우드의 세이브와 **어느 쪽이 최신인지 확인**하는 과정이 진행됩니다. 일부 게임은 날짜가 더 최근인 세이브를 선택해 올립니다.
- 오프라인 플레이가 길었다면, 온라인 복귀 직후 클라우드 파일과 충돌할 수 있으니 신중히 선택하시기 바랍니다.

게임에 따라 클라우드 저장을 항상 사용하도록 설정되어 있으므로, 오프라인 플레이가 많다면 세이브를 별도로 백업해두는 것이 좋습니다.
...
- 온라인으로 전환하면 대기 중이던 업데이트와 다운로드가 다시 시작됩니다.
