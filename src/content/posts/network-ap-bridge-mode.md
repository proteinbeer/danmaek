...
AP 모드는 보통 기존 공유기의 네트워크를 유선으로 받아 무선과 LAN을 확장하죠. 브리지 모드는 장비에 따라 통신사 모뎀 기능만 남기거나 무선으로 두 네트워크를 잇는 뜻으로 쓰일 수 있습니다.

라우터 모드 공유기를 두 대 겹치면 이중 NAT가 생길 수 있습니다. DHCP 서버가 두 개 켜지면 IP 충돌과 연결 문제를 만들 수 있죠.
...
사용 중인 공유기 제조사의 AP와 브리지 정의를 먼저 확인합니다. 두 번째 장비 WAN 주소가 사설 IP를 받고 있는지 봅니다. 집에서 포트포워딩, 게임 NAT, 내부 공유가 필요한지 목적을 정하죠.

여기서 중요한 것은 정상값을 하나 확보하는 것입니다. 예를 들어 다른 케이블, 다른 포트, 다른 PC처럼 정상 동작하는 조건을 하나 찾으면 문제 장비를 비교하기 쉬워집니다.

## 4. 실제로 해결하는 순서

단순 와이파이 확장이 목적이면 AP 모드를 우선 고려합니다. 통신사 장비 뒤 개인 공유기를 주 라우터로 쓰려면 통신사 장비의 브리지 지원을 확인하죠. DHCP와 NAT 역할은 특별한 이유가 없다면 한 장비에만 맡깁니다.
