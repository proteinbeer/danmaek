...
DNS 서버까지의 네트워크 경로가 내 통신사 환경에서 더 멀 수 있습니다. 콘텐츠 전송망이 DNS 위치를 기준으로 가까운 서버를 고르는 과정에서 불리해질 수 있습니다.

브라우저의 DNS over HTTPS와 윈도우 DNS가 서로 다른 경로를 쓰는 경우가 있습니다. 기존 캐시가 남아 변경 직후 결과가 들쭉날쭉할 수 있습니다.
...
기존 자동 DNS 상태에서 자주 쓰는 사이트 반응을 먼저 비교합니다. 1.1.1.1과 8.8.8.8을 각각 적용해 체감과 응답 차이를 봅니다. 다운로드 속도와 웹페이지 첫 연결 속도를 같은 문제로 판단하지 않습니다.

여기서 중요한 것은 정상값을 하나 확보하는 것입니다. 예를 들어 다른 케이블, 다른 포트, 다른 PC처럼 정상 동작하는 조건을 하나 찾으면 문제 장비를 비교하기 쉬워집니다.

## 4. 실제로 해결하는 순서

변경 뒤 문제가 생기면 DNS를 자동으로 되돌려 통신사 기본값과 비교합니다. 윈도우 DNS 캐시를 비운 뒤 브라우저도 다시 시작합니다. 특정 DNS에서만 느린 서비스가 있다면 다른 공용 DNS로 교체합니다.
...
DNS를 바꾼 뒤 속도 측정 숫자가 그대로라고 실패한 것은 아닙니다. 반대로 속도 측정이 빠르다고 DNS가 잘 맞는 것도 아니며, 주소 조회와 실제 데이터 전송을 분리해서 봐야 합니다.
