---
title: "윈도우 11 창 모드 게임 최적화, 전체 화면보다 나아질 수 있는 이유"
...
예전 PC 게임에서는 성능을 원하면 무조건 전체 화면으로 실행하라는 말이 흔했습니다.

윈도우 11에서는 창 모드와 테두리 없는 창 모드의 표시 방식이 개선되면서 오래된 공식이 항상 그대로 맞지는 않습니다.

윈도우에는 창 모드 게임 최적화라는 기능이 있습니다.

지원되는 다이렉트엑스 10과 11 게임의 창 모드 표시 방식을 개선해 입력 지연을 줄이고 현대적인 화면 기능을 사용할 수 있도록 돕습니다.

<img class="post-landscape-image" src="/assets/posts/guide-images/hero-bloom.jpg" alt="윈도우 11 그래픽 설정 화면" width="1280" height="720" loading="lazy" decoding="async" />
...
이 과정에서 자동 HDR과 VRR 같은 기능도 활용할 수 있는 조건이 만들어집니다.

테두리 없는 창 모드를 편하게 쓰면서 전체 화면과의 성능 차이를 줄이려는 기능이라고 이해하면 쉽습니다.
...
설정에서 시스템, 디스플레이, 그래픽으로 들어가면 기본 그래픽 설정에서 창 모드 게임 최적화를 확인할 수 있습니다.

기능을 켠 뒤에는 실행 중이던 게임을 다시 시작해야 적용 여부를 확인하기 쉽습니다.
...
듀얼 모니터로 디스코드, 공략, 방송 프로그램을 함께 쓰는 분에게는 이 편의성이 큽니다.
...
게임 최적화는 세대가 바뀌면서 달라집니다.

예전에 맞았던 설정 팁이 최신 윈도우와 최신 드라이버에서는 큰 의미가 없거나 반대가 되는 경우도 있습니다.

저는 전체 화면과 테두리 없는 창 모드 중 하나를 원칙처럼 고르기보다 실제 게임에서 프레임, 입력 지연, 화면 전환 편의성을 같이 보는 편이 낫다고 봅니다.
