...
마스크 크기와 광학 구조는 한 번에 얼마나 큰 칩 영역을 찍을 수 있는지에 영향을 줍니다.

하이NA EUV는 더 미세한 회로를 만들 수 있지만 현재 방식에서는 한 번에 처리할 수 있는 영역에 제약이 생깁니다.

작은 모바일 칩보다 거대한 AI 가속기에서 이 문제가 더 크게 보입니다.
...
미세 공정으로 트랜지스터를 줄여도 AI 연산 수요가 더 빠르게 늘면 칩은 다시 커집니다.
...
반도체 업계에서 장비 변화는 스마트폰 앱 업데이트처럼 빠르게 진행되지 않습니다.

마스크 규격을 바꾸면 장비뿐 아니라 재료와 검사, 설계 도구, 공정 전체가 함께 바뀌어야 합니다.

그래서 시험 라인부터 대량생산까지 여러 해가 필요합니다.
...
더 미세한 기술이 나왔다고 바로 쓰는 것이 항상 경제적이지는 않습니다.

기존 장비로 충분한 수율과 생산성을 얻을 수 있다면 기다리는 선택도 경쟁력입니다.
...
노광 장비는 한 대 가격이 막대해 시간당 처리량이 조금만 올라가도 공장 경제성에 큰 영향을 줍니다.
