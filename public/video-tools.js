...
function initVideoTools() {
    ...
    bindVideoAction('trimVideo', trimVideoFile, () => false);
...
function resetVideoTrimWaveform() {
    ...
    syncVideoTrimPreview(false, true);
}
...
function updateVideoTrimSelection() {
    ...
    syncVideoTrimPreview(videoTrimState.dragging);
}
...
function syncVideoTrimPreview(seekToStart = false, reset = false) {
    const preview = document.getElementById('videoTrimmerPreview');
    if (!preview) return;
    if (reset || !videoTrimState.duration) {
        preview.dataset.previewStart = '0';
        preview.dataset.previewEnd = '0';
        return;
    }
    const start = Math.max(0, Math.min(videoTrimState.start, videoTrimState.duration));
    const end = Math.max(start + 0.05, Math.min(videoTrimState.end, videoTrimState.duration));
...
function bindVideoTrimPreviewLoop() {
    const preview = document.getElementById('videoTrimmerPreview');
    if (!preview) return;
    preview.addEventListener('play', () => syncVideoTrimPreview(true));
...
function updateVideoGifEstimate() {
    ...
    output.textContent = `예상 파일 크기: ~${megabytes < 1 ? megabytes.toFixed(2) : megabytes.toFixed(1)} MB`;
    ...
    button.addEventListener('click', async () => {
        ...
        try {
            ...
            if (needsEngine(content)) {
                button.innerText = '엔진 로딩 중...';
                ffmpeg = await getVideoFfmpeg(content);
            }
            button.innerText = '작업 중...';
            ...
            setVideoStatus(button.closest('.calculator-content'), `작업 실패: ${detail.slice(0, 180)}`, true);
...
async function getVideoFfmpeg(content) {
    ...
    videoFfmpegLoading = (async () => {
        ...
        ffmpeg.on('progress', ({ progress }) => {
            setVideoStatus(content, `작업 중 ${Math.max(0, Math.min(100, Math.round(progress * 100)))}%`);
        });
...
async function videoToBlobURL(url, mimeType) {
    ...
    if (!response.ok) throw new Error(`비디오 엔진을 불러오지 못했습니다 (${response.status}).`);
...
async function compressVideoFile(ffmpeg, content) {
    ...
    setVideoStatus(content, '압축된 비디오를 다운로드했습니다.');
}
...
async function convertVideoFormat(ffmpeg, content) {
    ...
    setVideoStatus(content, `${extension.toUpperCase()} 비디오를 다운로드했습니다.`);
}
...
async function convertMp4ToWebmInBrowser(file, content) {
    ...
    try {
        ...
        const progressTimer = setInterval(() => {
            ...
            setVideoStatus(content, `변환 중 ${Math.max(0, Math.min(100, progress))}%`);
        }, 500);
        ...
        setVideoStatus(content, 'WEBM 비디오를 다운로드했습니다.');
    } finally {
        video.remove();
...
async function trimVideoFile(ffmpeg, content) {
    ...
    if (!file) return setVideoStatus(content, '비디오 파일을 추가해 주세요.', true);
    const start = videoTrimState.start;
    const end = videoTrimState.end;
    if (end <= start) return setVideoStatus(content, '끝 시간이 시작 시간보다 커야 합니다.', true);
    ...
    const captureMethod = HTMLMediaElement.prototype.captureStream || HTMLMediaElement.prototype.mozCaptureStream;
    if (!mimeType || !captureMethod) throw new Error('이 브라우저에서는 영상 자르기를 지원하지 않습니다.');
    ...
    try {
        await new Promise((resolve, reject) => {
            video.onloadedmetadata = resolve;
            video.onerror = () => reject(new Error('비디오 파일을 읽지 못했습니다.'));
        });
        ...
        await new Promise((resolve, reject) => {
            recorder.onerror = () => reject(new Error('영상 자르기 중 오류가 발생했습니다.'));
            recorder.onstop = resolve;
            recorder.start();
        ...
        const blob = new Blob(chunks, { type: mimeType });
        if (!blob.size) throw new Error('잘린 영상 파일이 비어 있습니다.');
        const outputUrl = URL.createObjectURL(blob);
        const link = document.createElement('a');
        link.href = outputUrl;
        link.download = buildVideoName(file.name, 'trimmed', 'webm');
        link.click();
        setTimeout(() => URL.revokeObjectURL(outputUrl), 1000);
    } finally {
        video.remove();
    ...
    setVideoStatus(content, '자른 비디오를 다운로드했습니다.');
}
...
async function createGifFromVideo(ffmpeg, content) {
    ...
    setVideoStatus(content, `GIF를 다운로드했습니다 (${(outputSize / 1024 / 1024).toFixed(2)} MB).`);
}
...
async function extractAudioFromVideo(ffmpeg, content) {
    ...
    setVideoStatus(content, `${format.toUpperCase()} 오디오를 다운로드했습니다.`);
}
...
function convertSubtitleFile() {
    ...
    reader.onload = () => {
        ...
        link.click();
        setTimeout(() => URL.revokeObjectURL(url), 1000);
        status.innerText = `${extension.toUpperCase()} 자막을 다운로드했습니다.`;
    };
